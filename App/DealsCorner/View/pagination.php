...
<?php if ($page !== 1): ?>        
    <a href="<?= urlWithPage($uri,$page - 1) ?>" class="page" data-page="<?= $page - 1 ?>"><span class="icon material-symbols-rounded hover">&#xe2ea;</span></a>
<?php endif; ?>
<div class="pages">
    <?php if ($pages <= 5):
        for ($i = 1; $i <= $pages; $i++): ?>
            <div class="page <?= $page == $i ? 'current' : ''; ?>" >
                <a href="<?= urlWithPage($uri,$i) ?>" data-page="<?= $i; ?>"><?= $i; ?></a>
            </div>
        <?php endfor;
    else: 
        if ($page > ($pages - 5)): // Dernières pages ?>
            <div class="page <?= $page == 1 ? 'current' : ''; ?>">
                <a href="<?= urlWithPage($uri,1) ?>" data-page="<?= 1; ?>">1</a>
            </div>
            ...
            <div class="page <?= $page == $pages ? 'current' : ''; ?>">
                <a href="<?= urlWithPage($uri,$pages) ?>" data-page="<?= $pages; ?>"><?= $pages; ?></a>     
            </div>
        <?php endif;
    endif; ?>
</div>
