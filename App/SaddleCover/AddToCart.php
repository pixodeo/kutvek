...
final class AddToCart extends AppAction {
	...
	public function __invoke(){		
		...
		$this->data['item']['sku'] = $this->data['item']['product'] . '.' . $this->data['item']['year'];
	...
	private function _designation(){
		...
		$year = (int)$this->data['item']['year'];
		$vehicle = $this->_vehicle($year);
    	return $this->specialchars_decode($designation . ' ' . $vehicle);
    }
    ...
    private function _vehicle(int $year): string{
    	$this->_orderTable->setEntity(null);
    	$sql = "SELECT CONCAT_WS(
    		' ', 
    		v.brand_name, 
    		v.name, 
    		CASE WHEN v_m.finish = '-' THEN CONCAT(v_m.begin, v_m.finish) 
    		ELSE CONCAT_WS('-', v_m.begin, v_m.finish) 
    		END) AS 'name'
			FROM vehicle_millesims v_m
			LEFT JOIN vue_vehicle_2 v ON v.id = v_m.vehicle AND v.l10n = 1
			WHERE v_m.id = :id";
		$q = $this->_orderTable->query($sql, ['id'=>$year], true);
		return $q ? $q->name : '';
    }
}