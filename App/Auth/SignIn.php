...
class SignIn extends Action {
	...
	public function __invoke(string $login, string $password) {
        ...
        try {
            ...
            if(!filter_var($login, FILTER_VALIDATE_EMAIL)) throw new Exception('17 - Wrong ids');
            $userInfo = $table->login($login);
            if(!$userInfo) throw new Exception('19 - Wrong ids, nobody'); 
            if($userInfo->pwd_hash === null  && $userInfo->pwd === null) throw new Exception(' 20 - Wrong ids'); 
            ...
            if(!password_verify($password, $pwd_hash)) throw new Exception('27 - Wrong ids');
