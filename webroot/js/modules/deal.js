const deal = {
  ...
  filters:async function(){
    this.refreshFilters();
    const behaviors = Array.from(document.querySelectorAll('input[name="behavior[]"]:checked'), (x) => x.value).join(',');
    ...
    if(behaviors.length > 0) urlParams.append('behaviors', behaviors);
