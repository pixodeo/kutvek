...
const customPlate = {
    ...
    fetch(url, { method: 'GET' })
      ...
      .then((json) => {
        if (json.brands && json.brands.length > 0) {
          ...
          let platesQuad = document.getElementById('quad');
          let platesMx = document.getElementById('mx');
          let inputPlateMx = document.getElementById('custom-mx-plate-front');
          if (familly == 2) {            
            platesQuad.classList.remove('hide');
            platesMx.classList.add('hide');
            inputPlateMx.required = false;
            inputPlateMx.disabled = true;

          } else {
            platesQuad.classList.add('hide');
            platesMx.classList.remove('hide');
            inputPlateMx.required = true;
            inputPlateMx.disabled = false;
          }
        }
      }
      );
  ...
  // Ajouter les élements de type checkbox au panier
  ...
  // Ajouter les élements de type select au panier
  ...
  update: function (qty) {
    ...
    var updated = (price + finishCost  + premiumCost +  accessories) * qty;
